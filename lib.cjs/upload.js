...
exports.BaseUpload = exports.defaultOptions = void 0;
...
exports.defaultOptions = {
    ...
    httpStack: undefined,
    protocol: options_js_1.PROTOCOL_TUS_V1,
};
class BaseUpload {
    ...
    start() {
        ...
        if (![options_js_1.PROTOCOL_TUS_V1, options_js_1.PROTOCOL_IETF_DRAFT_03, options_js_1.PROTOCOL_IETF_DRAFT_05].includes(this.options.protocol)) {
    ...
    async _prepareAndStartUpload() {
        ...
        if (this._fingerprint == null) {
            (0, logger_js_1.log)('No fingerprint was calculated meaning that the upload cannot be stored in the URL storage.');
        }
        else {
            (0, logger_js_1.log)(`Calculated fingerprint: ${this._fingerprint}`);
        }
    ...
    async _startParallelUpload() {
        ...
        try {
            ...
            throw new DetailedError_js_1.DetailedError('tus: failed to concatenate parallel uploads', err, req, undefined);
        }
        if (!inStatusCategory(res.getStatus(), 200)) {
            throw new DetailedError_js_1.DetailedError('tus: unexpected response while creating upload', undefined, req, res);
        }
        const location = res.getHeader('Location');
        if (location == null) {
            throw new DetailedError_js_1.DetailedError('tus: invalid or missing Location header', undefined, req, res);
        }
        if (this.options.endpoint == null) {
            throw new Error('tus: Expeced endpoint to be defined.');
        }
        this.url = resolveUrl(this.options.endpoint, location);
        (0, logger_js_1.log)(`Created upload at ${this.url}`);
    ...
    async _startSingleUpload() {
        ...
        if (this.url != null) {
            (0, logger_js_1.log)(`Resuming upload from previous URL: ${this.url}`);
            return await this._resumeUpload();
        }
        // A URL has manually been specified, so we try to resume
        if (this.options.uploadUrl != null) {
            (0, logger_js_1.log)(`Resuming upload from provided URL: ${this.options.uploadUrl}`);
            this.url = this.options.uploadUrl;
            return await this._resumeUpload();
        }
        // An upload has not started for the file yet, so we start a new one
        (0, logger_js_1.log)('Creating a new upload');
    ...
    async _createUpload() {
        ...
        try {
            if (this.options.uploadDataDuringCreation && !this._uploadLengthDeferred) {
                ...
                if (this.options.protocol === options_js_1.PROTOCOL_IETF_DRAFT_03 ||
                    this.options.protocol === options_js_1.PROTOCOL_IETF_DRAFT_05) {
            ...
            throw new DetailedError_js_1.DetailedError('tus: failed to create upload', err, req, undefined);
        }
        if (!inStatusCategory(res.getStatus(), 200)) {
            throw new DetailedError_js_1.DetailedError('tus: unexpected response while creating upload', undefined, req, res);
        }
    ...
    async _resumeUpload() {
        ...
        try {
            ...
            throw new DetailedError_js_1.DetailedError('tus: failed to resume upload', err, req, undefined);
        }
        ...
        if (!inStatusCategory(status, 200)) {
            ...
            // afterwards.
            if (status === 423) {
                throw new DetailedError_js_1.DetailedError('tus: upload is currently locked; retry later', undefined, req, res);
            }
            ...
            if (!this.options.endpoint) {
                ...
                throw new DetailedError_js_1.DetailedError('tus: unable to resume upload (new upload cannot be created without an endpoint)', undefined, req, res);
            }
        ...
        if (offsetStr === undefined) {
            throw new DetailedError_js_1.DetailedError('tus: missing Upload-Offset header', undefined, req, res);
        }
        const offset = Number.parseInt(offsetStr, 10);
        if (Number.isNaN(offset)) {
            throw new DetailedError_js_1.DetailedError('tus: invalid Upload-Offset header', undefined, req, res);
        }
        ...
        if (Number.isNaN(length) &&
            ...
            this.options.protocol === options_js_1.PROTOCOL_TUS_V1) {
            throw new DetailedError_js_1.DetailedError('tus: invalid or missing length value', undefined, req, res);
        }
        if (typeof this.options.onUploadUrlAvailable === 'function') {
            await this.options.onUploadUrlAvailable();
        }
    ...
    async _performUpload() {
        ...
        try {
            ...
            throw new DetailedError_js_1.DetailedError(`tus: failed to upload chunk at offset ${this._offset}`, err, req, undefined);
        }
        if (!inStatusCategory(res.getStatus(), 200)) {
            throw new DetailedError_js_1.DetailedError('tus: unexpected response while uploading chunk', undefined, req, res);
        }
        await this._handleUploadResponse(req, res);
    }
    /**
    ...
    async _addChunkToRequest(req) {
        ...
        if (this.options.protocol === options_js_1.PROTOCOL_TUS_V1) {
            req.setHeader('Content-Type', 'application/offset+octet-stream');
        }
        else if (this.options.protocol === options_js_1.PROTOCOL_IETF_DRAFT_05) {
    ...
    async _handleUploadResponse(req, res) {
        ...
        if (Number.isNaN(offset)) {
            throw new DetailedError_js_1.DetailedError('tus: invalid or missing offset value', undefined, req, res);
        }
...
function encodeMetadata(metadata) {
    return Object.entries(metadata)
        .map(([key, value]) => `${key} ${js_base64_1.Base64.encode(String(value))}`)
        .join(',');
}
/**
...
function openRequest(method, url, options) {
    ...
    if (options.protocol === options_js_1.PROTOCOL_IETF_DRAFT_03) {
        req.setHeader('Upload-Draft-Interop-Version', '5');
    }
    else if (options.protocol === options_js_1.PROTOCOL_IETF_DRAFT_05) {
    ...
    if (options.addRequestId) {
        const requestId = (0, uuid_js_1.uuid)();
...
function resolveUrl(origin, link) {
    return new url_parse_1.default(link, origin).toString();
}
/**
...
async function terminate(url, options) {
    ...
    try {
        ...
        throw new DetailedError_js_1.DetailedError('tus: unexpected response while terminating upload', undefined, req, res);
    }
    catch (err) {
        if (!(err instanceof Error)) {
            throw new Error(`tus: value thrown that is not an error: ${err}`);
        }
        const detailedErr = err instanceof DetailedError_js_1.DetailedError
            ? err
            : new DetailedError_js_1.DetailedError('tus: failed to terminate upload', err, req);
